...
$(document).ready(function () {
    ...
    userData = JSON.parse(sessionStorage.getItem("userLogin")); //שליפת מייל של הדייר המחובר
    emailUserLogin = userData.email;
    ...
    if (
        ...
        url = "https://localhost:7006/api/User/CurrentNumberOfCoins/";
    } else {
        url = "https://proj.ruppin.ac.il/cgroup44/prod/api/User/CurrentNumberOfCoins/";
    }

    ajaxCall("GET", url + emailUserLogin, "", getDetilsOfCoinsSCB, getDetilsOfCoinsECB); //שליפת מטבעות ובקשות שיש למשתמש
});

function getDetilsOfCoinsSCB(data) {
    //כמות בקשות שנותר למשתמש לבקש
    NumOfRequestsAllowed = data.coins - data.numOfRequest;
}

function getDetilsOfCoinsECB(error) {
    console.log(error)
}

function checkBorrows() {// שליפת חניות מתאימות
    //קודם תפיסת נתונים מהפקדים כלומר מה הבקשה שלו
...
function Income(idB) {// הכנסה של הבקשה לבסיס הנתונים לא משנה אם זה בשביל הזמנה או הבקשה הכללית של הבניין
    if (NumOfRequestsAllowed > 0) {
        ...
        } else IdBorrowNow = undefined
        
        if (IdBorrowNow == undefined) {
            IdBorrowNow = 0;
        }
        NumOfRequestsAllowed = NumOfRequestsAllowed - 1; //עדכון שלקח בקשה נוספת
        ajaxCall("GET", api + "/CheckingRequestBeforInsert?idBorrow=" + IdBorrowNow + "&email_request=" + emailUserLogin + "&startDate=" + Request.startDate + "&endDate=" + Request.endDate + "&startTime=" + Request.startTime + "&endTime=" + Request.endTime, "", GetCheckingRequestSCB, GetCheckingRequestECB);//בדיקה שאין הזמנה לאותה החניה על ידי אותו הדייר
    }
    else {
        MessageToUser('כשלון', 'אין לך מספיק מטבעות כדי לבצע פעולה זו, ניתן להזמין חניות לפי כמות המטבעות');
    }
}

function GetCheckingRequestSCB(data) {  //במידה ועמד בבדיקה- כלומר אין בקשה לאותה החניה על ידי אותו הדייר
    if (data == 0) {
...
function GetCheckingRequestECB(error) {  //קיימת כבר בקשה לאותה החניה על ידי אותו הדייר
    MessageToUser('כשלון', 'שיים לב! קיימת לך כבר בקשה לחנייה זו, בקשה מספר: ' + error +' לא ניתן לבקש את אותה החניה מספר פעמים ');
}
