...
namespace Final_Project_Rev1.Models
{
    public class Borrow
    {
        ...
        public List<object> UserBorrows(string mail)  //שליפת השאלות לפי משתמש
        {
            DBservices dbs = new DBservices();
            List<object> userBorrow = new List<object>();  //הרשימה שתחזור לשרת edennnnnn
            ...
            for (int i = 0; i < UBorrows.Count; i++) // בודקת כל השאלה עם היא מלאה או לא
            {
                ...
                //השמה של ההשאלה לאובייקט כללי
                object borrowWithBar = new object();
                borrowWithBar = ProgressBar(UBorrows[i], approvedMatches);
                userBorrow.Add(borrowWithBar);
            }
            return userBorrow;
        }

        //מוסיפות עמודה של בר התקדמות כמה מלאה ההשאלה
        public object ProgressBar(Borrow Borrow, List<object> approvedMatches)
        {
            double totalUtilizationTime = 0;
            if (approvedMatches.Count > 0)
            {
                for (int i = 0; i < approvedMatches.Count; i++)
                {
                    var approvedMatch = (dynamic)approvedMatches[i];
                    //סידור תאריך וזמן של בקשה במשתנה אחד
                    DateTime requestStart = approvedMatch.RequestStartDate.Date + approvedMatch.RequestStartTime.TimeOfDay;
                    DateTime requestEnd = approvedMatch.RequestEndDate.Date + approvedMatch.RequestEndTime.TimeOfDay;
            ...
            DateTime borrowStart = Borrow.StartDate.Date + Borrow.StartTime.TimeOfDay;
            DateTime borrowEnd = Borrow.EndDate.Date + Borrow.EndTime.TimeOfDay;
            double borrowTotalTime = (borrowEnd - borrowStart).TotalHours; //חישוב זמן כולל של ההשאלה
            double ProgressBar = Math.Round((totalUtilizationTime / borrowTotalTime) * 100);  //סך הניצולת 
            object borrowNew = new
            {
                Id = Borrow.id,
        ...
        //eden
        public List<object> GetBorrowForGeneral(int idBuilding, string email) //מחזיר את כל ההשאלות הפנויות לדף הכללי
        {
            DBservices dbs = new DBservices();
            return dbs.ReadBorrowForGeneral(idBuilding, email);   //כל השאלות שהן עדיין פנויות לדף בקשות כללי   
        }
        //eden
    }
}
