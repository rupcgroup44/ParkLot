...
namespace Final_Project_Rev1.Models
{
    public class Email
    {
        ...
        {
            string from = "parkinglotprojct@gmail.com";  //ממי הוא שולח- אנחנו
            string password = "hlul ahhi lqvu cylu"; // קבלת סיסמה מהגדרות של אימייל
            string host = "smtp.gmail.com"; // replace with your SMTP server address השרת ששולח את המיל 

            MailMessage message = new MailMessage();
            message.From = new MailAddress("parkinglotprojct@gmail.com"); // replace with a valid email address
            message.To.Add(email);  //נמען
            message.Subject =strSubject;  //כותרת
            message.Body = strBody;   //גוף ההודעה
        ...
        public int DeleteRequest(object data)
        {
            var match = (dynamic)data; // המרה לאובייקט דינמי כדי שנוכל לגשת לשדות
            string RequestStartDateString = match.RequestStartDate.ToString("dd/MM/yyyy"); //בדיקה
            string RequestEndTimeString = match.RequestStartTime.ToString("hh:mm:ss tt");
            string RequestStartTimeString = match.RequestEndTime.ToString("hh:mm:ss tt");
            string strsubject = "בקשה שאישרת בוטלה -ParkLot";
            string str = "הבקשה של: " + match.RFirstName +" "+ match.RFamilyName + " בוטלה ";
            str += "\n";
            str += "בתאריך: " + RequestStartDateString;
            str += "\n";
            str += "בין השעות: " + RequestStartTimeString + " " + RequestEndTimeString;
            str += "\n";
            str += "כעת ההשאלה שלך שוב זמינה לבקשות נוספות";
            str += "\n";
            str += "יום טוב, צוות Park-Lot";
            return SendEmail(match.BorrowMail, strsubject, str);
        }

        public int PasswordUser(string EmailAddress,string Password)  //שליחת מייל לאחר הרשמה לאתר
        {
            string strsubject = "סיסמא לאתר Park-Lot";
            string strbody = "לצורך התחברות לאתר הסיסמא איתה נרשמת הינה: " +Password;
            return SendEmail(EmailAddress, strsubject, strbody);
        }
