...
namespace Final_Project_Rev1.Models
{
    ...
    {    
        public List<object> getBorrowDetails(int idBorrow) //מחזיר את כל הבקשות המתאימות להשאלה
        {
            ...
            int BuildingID = dbs.GetBuildingID(idBorrow);  //שליפת מספר אידי של בניין כדי שיתבצע רק על אותו הבניין
            ...
            if (selectedCombination != null)  //רק אם חזרה רשימה
            {
                for (int i = 0; i < selectedCombination.Count; i++)
                {
                    ...
                    FinalCombination.Add(requestWithName);  //מחזיר אובייקט שמכיל פרטי בקשה וגם שם מלא של המשתמש וכוכבים
                }
            } 
        ...
        public List<List<Request>> GenerateRequestCombinations(Borrow selectedBorrow, List<Request> requestList) //פונקצית מעבר לפונקציה הבאה
        {
        ...
        private void GenerateCombinationsRecursive(Borrow selectedBorrow, List<Request> requestList, List<List<Request>> combinations, List<Request> currentCombination, int index)
        {
            ...
            if (currentCombination.Count > 0 && IsCombinationValid(selectedBorrow, currentCombination))  //אם הקומבינציה הנוכחית טובה
            {
            ...
            for (int i = index; i < requestList.Count; i++)
            {
                ...
                GenerateCombinationsRecursive(selectedBorrow, requestList, combinations, currentCombination, i + 1);  //קריאה רקורוסיבית
                currentCombination.RemoveAt(currentCombination.Count - 1);  //מסירה את האחרון
            }
        }
        ...
        private bool IsCombinationValid(Borrow selectedBorrow, List<Request> combination)
        {
            ...
            DateTime borrowStart = selectedBorrow.StartDate.Date + selectedBorrow.StartTime.TimeOfDay;  //מעביר לפורמט dateTime
            ...
            for (int i = 0; i < combination.Count; i++)
            {
                ...
                DateTime requestStart1 = combination[i].StartDate.Date + combination[i].StartTime.TimeOfDay; //מעביר לפורמט dateTime
                ...
                for (int j = i + 1; j < combination.Count; j++) 
                {
                    ...
                    {
                        return false;  //קומבינציה לא טובה- בקשות חופפות
                    }
                }
            }

            return true;  //קומבינציה טובה
        }
        ...
        public List<Request> GetCombinationWithMostUtilization(List<List<Request>> combinations, Borrow selectedBorrow)
        {
            ...
            foreach (var combination in combinations)
            {
                ...
                //בדיקה האם הקומבינציה שבדקנו עכשיו היא המקסימלית ובנוסף שהזמן לא חורג מזמן ההשאלה
        ...
        public int UpdateToSmartAlgorithem(int idBorrow) //בחירת שיבוץ חכם בעקבות כך -עדכון משתמשים נעשה בהתאם למצבם
        {
            ...
            BestAlgorithem = getBorrowDetails(idBorrow);  //פרטי השאלה
            
            List<object> Match = new List<object>();  //מגדירה רשימת מאצים
            DBservices dbs = new DBservices();
